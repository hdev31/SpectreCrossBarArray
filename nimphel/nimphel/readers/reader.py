...
from nimphel.core import Element, Instance, Directive, Subcircuit, Circuit
from typing import Union, List, Optional, IO, Type, TextIO


class BaseReader(ABC):
    "Basic Interface for an Element Reader"

    @abstractmethod
    def load(self, source: Union[str, bytes, bytearray], *args, **kwargs) -> object:
        pass

    @abstractmethod
    def load_from_file(self, fp: IO, *args, **kwargs) -> object:
        self.load(fp.read())


class Reader(ABC):
    """Deserialize object to Circuit"""

    def load(self, source: Union[str, bytes, bytearray], *args, **kwargs) -> Element:
        pass
...
class Parser(Reader):
    ...
    def parse(self, source: str, path: Optional[str] = None) -> Circuit:
...
class SpectreReader(Parser):
    def __init__(self, *args, **kwargs):
        super().__init__("spectre", *args, **kwargs)


class VerilogAReader(Parser):
    def __init__(self, *args, **kwargs):
        super().__init__("veriloga", *args, **kwargs)


class EldoReader(Parser):
    def __init__(self, *args, **kwargs):
        super().__init__("eldo", *args, **kwargs)
